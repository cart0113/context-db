...
build_one() {
    ...
    # ── Read optional instructions prose ─────────────────────────────────────

    local instructions_content=""
    if [ -f "$context_dir/CONTEXT_INSTRUCTIONS.md" ]; then
        instructions_content="$(cat "$context_dir/CONTEXT_INSTRUCTIONS.md")"
    fi
    ...
    for md_file in "$context_dir"/*.md; do
        ...
        case "$fname" in
            CONTEXT_TOC.md|CONTEXT_INSTRUCTIONS.md|SYSTEM.md) continue ;;
        esac
