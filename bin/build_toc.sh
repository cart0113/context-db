...
# Read description — tries fenced ```yaml description``` block, then front matter
read_desc() {
    local desc
    desc=$(awk '
...
build_dir() {
    ...
    if [ -n "$folder_lines" ]; then
        ...
        folder_table="_No subfolders._"
    fi
    if [ -n "$file_lines" ]; then
        file_table="| File | Description |"$'\n'"| ---- | ----------- |${file_lines}"
    else
        file_table="_No files._"
    fi

    {
        printf '%s\n' \
            "<!-- Auto-generated by build_toc.sh — edit ${desc_fname} instead -->" \
            "" \
            "# ${foldername}" \
            ""
